...
public class Messenger extends AppCompatActivity {
                       ...
                       for(DocumentChange documentChange:queryDocumentSnapshots.getDocumentChanges()){
                           ...
                           if(documentChange.getType()==ADDED){
                               ...
                               if((receiver_id.equalsIgnoreCase(Messenger.this.sender_id)||receiver_id.equalsIgnoreCase(Messenger.this.receiver_id))&&!sender_id.equalsIgnoreCase(SharedPrefManager.getInstance(getApplicationContext()).getUser().user_id)){
                                   ...
                                   String time=DateTimeConverter.getInstance().get_current_date_time2();
    ...
    public void set_own_message_data(String message_id,String message,String sender_id,String receiver_id,String sender_type,String receiver_type,String time){
        ...
        else if(SharedPrefManager.getInstance(getApplicationContext()).getUser().user_type.equalsIgnoreCase("Doctor")){
            sender_tmp="Doctor";
        }
        else if(SharedPrefManager.getInstance(getApplicationContext()).getUser().user_type.equalsIgnoreCase("admin")){
            sender_tmp="Admin";
        }
        NotificationSender.getInstance(getApplicationContext()).createNotification(sender_tmp+" আপনাকে মেসেজ পাঠিয়েছেন।",message,sender_id,sender_name,sender_image_path,SharedPrefManager.getInstance(getApplicationContext()).getUser().user_type,receiver_id,message_id,sender_device_id,receiver_device_id,"new message");
    }
}
